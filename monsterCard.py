...
def monsterStatCard(monsterJson):
    pg.init()
    TITLE = pg.font.Font('freesansbold.ttf',32)
...
##    DISPLAYSURF = pg.display.set_mode((WINDOWWIDTH,WINDOWHEIGHT),HWSURFACE | DOUBLEBUF|RESIZABLE)
    ...
    DISPLAYSURF.blit(monsterName, (int(screen.get_width()/2-monsterName.get_width()/2), int(screen.get_height()/10-30) ))
    DISPLAYSURF.blit(monsterName, (int(screen.get_width()/2-monsterName.get_width()/2), int(screen.get_height()*9/10-15) ))
    # Putting AC on Card
