...
def createClasses(arguments):
    ...
    template = pg.transform.smoothscale(template, (int(template.get_width()/8*WINDOWWIDTH/1000),int(template.get_height()/8*WINDOWHEIGHT/629)))
    classRects = []
    ...
    for i in range(0,2):
        for j in range(0,4):            
            ...
            position = (int(surf.get_width()//3+template.get_width()*(i)),int(surf.get_height()/5+template.get_height()*(j)))
            surf.blit(template, position)
            classRects.append(template.get_rect(topleft = position))
            surf.blit(names[j+i*4], (position[0]+template.get_width()/2-names[j+i*4].get_width()/2, position[1]+template.get_height()/2-names[j+i*4].get_height()/2))
...
def spellClassFilter(arguments):
    ...
    if keyPrev == 'class':
        ...
        with open(rosterPath, 'r') as roster:
            for line in roster:
                currentPlace = line[:-1].split(',')
                if spellClass in currentPlace[1]:
                    spellNames.append(currentPlace[0].replace('[','').replace("'","").replace('_','').replace('"',''))
