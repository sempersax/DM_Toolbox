...
def spellCard(arguments):
    ...
    screen = pg.transform.scale(screen, (
    WINDOWWIDTH, WINDOWHEIGHT))  # (int(screen.get_width()*1.6),int(screen.get_height()*1.15)))
    ...
    DISPLAYSURF.blit(spellName,
                     (int(screen.get_width() / 2 - spellName.get_width() / 2), int(screen.get_height() / 10 - 40)))
    DISPLAYSURF.blit(spellName,
                     (int(screen.get_width() / 2 - spellName.get_width() / 2), int(screen.get_height() * 9 / 10 - 25)))
    ...
    DISPLAYSURF.blit(spellComponents, (
    int(screen.get_width() - spellRangeLabel.get_width() * 8.2 + spellComponentsLabel.get_width()),
    int(screen.get_height() / 10 * 2 + spellRangeLabel.get_height())))

    if materials != '':
        # Putting materials on Card
        DISPLAYSURF.blit(spellMaterialsLabel,
                         (edge, int(screen.get_height() / 10 * 2 + 4 * spellLevelLabel.get_height())))
        DISPLAYSURF.blit(spellMaterials, (
        edge + spellMaterialsLabel.get_width(), int(screen.get_height() / 10 * 2 + 4 * spellLevelLabel.get_height())))
        shift = 6
    else:
        shift = 4
    try:
        DISPLAYSURF.blit(spellEffectLabel,
                         (edge, int(screen.get_height() / 10 * 2 + spellLevelLabel.get_height() * shift)))
        shift += 1
        for i in range(0, len(spellEffects)):
            DISPLAYSURF.blit(spellEffects[i],
                             (edge, int(screen.get_height() / 10 * 2 + spellLevelLabel.get_height() * shift + 3)))
            shift += 1
    except:
        pass
